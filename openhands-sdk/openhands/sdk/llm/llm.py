...
class LLM(BaseModel, RetryMixin, NonNativeToolCallingMixin):
    ...
    def responses(
        ...
        **kwargs,
        ...
        Note:
            ...
            explainability of agent actions.
        """
        # Streaming not yet supported for non-Codex transports
        if not (
            "chatgpt.com" in (self.base_url or "").lower()
            and "backend-api" in (self.base_url or "").lower()
            and "codex" in (self.base_url or "").lower()
        ):
        ...
        def _one_attempt(**retry_kwargs) -> ResponsesAPIResponse:
            ...
            with self._litellm_modify_params_ctx(self.modify_params):
                with warnings.catch_warnings():
                    ...
                    # Codex subscription transport forces streaming; consume stream
                    # and return the completed response payload.
                    completed = None
                    for event in ret:
                    ...
                    if completed is None:
                        raise ValueError(
                            f"Streaming response did not yield completion: {type(ret)}"
                        )
                    self._telemetry.on_response(completed)
                    return completed
    ...
    def _get_codex_instructions(self) -> str:
        """Fetch official Codex instructions from OpenAI's GitHub repo."""
        cache_dir = os.path.join(os.path.expanduser("~"), ".openhands", "cache")
        cache_path = os.path.join(cache_dir, "codex-instructions.md")
        cache_meta_path = os.path.join(cache_dir, "codex-instructions-meta.json")
        ...
        if os.path.exists(cache_meta_path):
            try:
                with open(cache_meta_path, "r", encoding="utf-8") as meta_file:
                    meta = json.load(meta_file)
                cached_etag = meta.get("etag")
                cached_timestamp = meta.get("last_checked")
        ...
        if (
            cached_timestamp
            and time.time() - cached_timestamp < cache_ttl_seconds
            and os.path.exists(cache_path)
        ):
            with open(cache_path, "r", encoding="utf-8") as cache_file:
                return cache_file.read()

        prompt_url = "https://raw.githubusercontent.com/openai/codex/rust-v0.81.0/codex-rs/core/gpt-5.2-codex_prompt.md"
        headers = {}
        if cached_etag:
            headers["If-None-Match"] = cached_etag

        try:
            response = requests.get(prompt_url, headers=headers, timeout=10)
            if response.status_code == 304 and os.path.exists(cache_path):
                with open(cache_path, "r", encoding="utf-8") as cache_file:
                    return cache_file.read()
        ...
        with open(cache_path, "w", encoding="utf-8") as cache_file:
            cache_file.write(instructions)
        with open(cache_meta_path, "w", encoding="utf-8") as meta_file:
            json.dump(
                {
    ...
    def format_messages_for_responses(
        ...
        Codex subscription endpoints require the official Codex instructions
        prompt in the `instructions` field. When using the ChatGPT subscription
        transport (chatgpt.com/backend-api/codex), we also prepend any system
        prompts to the first user message for compatibility with the Codex CLI.
        """
        ...
        # For Codex subscription, fetch official instructions from OpenAI's GitHub repo.
        # The instructions must be the FULL official Codex prompt, not a short header.
        DEFAULT_CODEX_INSTRUCTIONS = (
            self._get_codex_instructions() if is_subscription_codex_transport else None
        )
        ...
        # OpenCode sends content as plain strings, not arrays of {type: input_text}
        if is_subscription_codex_transport and input_items:
            normalized: list[dict[str, Any]] = []
            for item in input_items:
                if item.get("type") == "message":
                    # Convert content array to plain string
                    content = item.get("content") or []
                    if isinstance(content, list):
                        # Extract text from input_text items
                        text_parts = []
                        for c in content:
